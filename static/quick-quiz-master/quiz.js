...
solve
        ...
        ans_btn.on('click', function() {
          ...
          function next() {
            ...
            // keep track in total
            if (correct) {
                quiz_answered=quiz_answered+"o";
                state.correct++;
            } else{
              quiz_answered=quiz_answered+"x";
            }
            $quiz.carousel('next');
    ...
    $("<button>")
      ...
      .click(function() {
        // $.post("/posttestresult",{
        //   javascript_data: state
        // });
        // $.get("/getdata/<state8>");

        window.location = "/getdata?a="+window.location.pathname+"|"+state.correct+"|"+state.total+"|"+quiz_answered+"|"+pu_id;
      })
      .appendTo($restart_button);
    //---------------
  ...
  function resultsText(state) {
    ...
    //     break;
    // }
    text = state.correct + " out of " + state.total
    return text;
  ...
  function tweet(state, opts) {
    ...
    var body = (
      "I got " + state.correct +
      " out of " + state.total +
      " on @Genesis Academy’s \"" + opts.title +
      "\" e-Learning Centre. Learn more at http://www.genesisacademy.com.au"
    );
  ...
  //----jeremy added
  // function test_finished(state,opts) {
  //   return "/dashboard"
  // }
